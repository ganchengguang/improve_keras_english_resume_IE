...
from tensorflow.keras.layers import Embedding, SpatialDropout1D,LSTM,  Dense, Bidirectional, GRU ,Dropout
...
class WordVecLstmSigmoid(object):
    ...
    def create_model(self):
        ...
        self.model.add(
            Bidirectional(GRU(units=64, dropout=0.2, recurrent_dropout=0.2, return_sequences=True,input_shape=(self.max_len, embedding_size))))
...
class WordVecLstmSoftmax(object):
    ...
    def create_model(self):
        ...
        self.model.add(LSTM(units=64, dropout=0.5, recurrent_dropout=0.5))
        self.model.add(Dense(len(self.labels), activation='softmax'))
...
class WordVecBidirectionalLstmSoftmax(object):
    ...
    def create_model(self):
        ...
        self.model.add(SpatialDropout1D(0.5))
        self.model.add(
            Bidirectional(LSTM(units=64, dropout=0.5,recurrent_dropout=0.5,return_sequences=True,input_shape=(self.max_len, embedding_size))))
        self.model.add(Attention(32))
        self.model.add(Dense(len(self.labels), activation='softmax'))
